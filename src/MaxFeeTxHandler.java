...
class MaxFeeTxHandler {
    ...
    private double calTxFee(Transaction tx) {
        double inputValue = 0, outputValue = 0;
        for (int i = 0; i < tx.numInputs(); i++) {
            Transaction.Input input = tx.getInput(i);
            UTXO utxo = new UTXO(input.prevTxHash, input.outputIndex);
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        // IMPLEMENT THIS
        Set<Transaction> acceptedTxs = new TreeSet<>((tx1, tx2) -> {
            double feeTx1 = calTxFee(tx1);
            double feeTx2 = calTxFee(tx2);
            return Double.compare(feeTx1, feeTx2);
        });

        Collections.addAll(acceptedTxs, possibleTxs);
